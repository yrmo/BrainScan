...
def main(
    ...
    validation_dataset = create_dataset(
        filenames=valid_list,
        mode="validation",
        num_epochs=1,
        batch_size=size_batch_valid,
        perform_one_hot=perform_one_hot,
        binarize_labels=binarize_labels,
    )
    ...
    training_placeholder = tf.placeholder(
        dtype=tf.bool, shape=[], name="training_placeholder"
    )
    ...
    if label_input_size > 1:  # OneHotEncoding
        ...
        if label_output_size == 1:
            loss_op = tf.reduce_mean(
                tf.losses.sigmoid_cross_entropy(multi_class_labels=y, logits=logits)
            )
        else:
            loss_op = tf.reduce_mean(
    ...
    if label_input_size > 1:  # OneHotEncoding
        ...
        IoU_metrics, IoU_metrics_update = tf.metrics.mean_iou(
            labels=y,
            predictions=logits_soft,
            num_classes=label_input_size,
            name="my_metric_IoU",
        )
        # Isolate the variables stored behind the scenes by the metric operation
        running_vars = tf.get_collection(
            tf.GraphKeys.LOCAL_VARIABLES, scope="my_metric_IoU"
        )
    ...
    train_op = tf.group(
        [update_ops, optimizer.minimize(loss_op, global_step=global_step)]
    )
    ...
    if label_input_size == 1:
        ...
        tf.summary.image(
            "labels_0", tf.expand_dims(tf.cast(y, tf.float32)[:, :, :, 0], axis=-1)
        )
        tf.summary.image(
            "labels_1", tf.expand_dims(tf.cast(y, tf.float32)[:, :, :, 1], axis=-1)
        )
        if label_input_size > 2:
            tf.summary.image(
                "labels_2", tf.expand_dims(tf.cast(y, tf.float32)[:, :, :, 2], axis=-1)
            )
            tf.summary.image(
                "labels_3", tf.expand_dims(tf.cast(y, tf.float32)[:, :, :, 3], axis=-1)
            )
    ...
    if label_output_size == 1:
        ...
        tf.summary.image(
            "prediction_0", tf.expand_dims(logits_soft[:, :, :, 0], axis=-1)
        )
        tf.summary.image(
            "prediction_1", tf.expand_dims(logits_soft[:, :, :, 1], axis=-1)
        )
        if label_output_size > 2:
            tf.summary.image(
                "prediction_2", tf.expand_dims(logits_soft[:, :, :, 2], axis=-1)
            )
            tf.summary.image(
                "prediction_3", tf.expand_dims(logits_soft[:, :, :, 3], axis=-1)
            )
    ...
    writer_val = tf.summary.FileWriter(
        os.path.join(logdir, "validation loss"), graph=tf.get_default_graph()
    )
        ...
        for epoch in range(num_epochs):
            ...
            try:
                while True:
                    ...
                    # train
                    _, cost, summary_val, step_gl, logits_val, _ = sess.run(
                        [
                            train_op,
                            loss_op,
                            summary_op,
                    ...
                    step += 1
                    print(
                        "\n Training step: Epoch {}, batch {} -- Loss: {:.3f}".format(
                            epoch + 1, step, cost
                        )
                    )
                    ...
                    if step % step_metrics == 0:
                        total_validation_loss = (
                            []
                        )  # list where we will store the loss at each batch
                        ...
                        try:
                            ...
                            while True:
                                if label_input_size > 1:  # OneHotEncoding
                                    cost_valid, _ = sess.run(
                                        [loss_op, IoU_metrics_update],
                                        feed_dict={
                        ...
                        validation_loss_summary = tf.Summary(
                            value=[
                                tf.Summary.Value(
                                    tag="loss", simple_value=total_validation_loss
                                )
                            ]
                        )
                        ...
                        if label_input_size > 1:  # OneHotEncoding
                            ...
                            IoU_summary = tf.Summary(
                                value=[
                                    tf.Summary.Value(
                                        tag="IoU_metrics", simple_value=IoU_score
                                    )
                                ]
                            )
                            writer.add_summary(IoU_summary, step_gl)
                            print(
                                "\n Epoch {} and training batch {} -- Validation loss {:.3f} and IoU metrics {:.3f}".format(
                                    epoch + 1, step, total_validation_loss, IoU_score
                                )
                            )
                        else:
                            print(
                                "\n Epoch {} and training batch {} -- Validation loss {:.3f}".format(
                                    epoch + 1, step, total_validation_loss
                                )
                            )
                    ...
                    if step % steps_saver == 0:
                        print(
                            "\n Step {} Saving weights to {}".format(
                                step + 1, model_checkpoint_path
                            )
                        )
                        saver.save(
                            sess,
