...
class GenerateTFRedord:
    ...
    def get_gray_scale(self, data):
        data = (
            (
                np.subtract(data[:, :], data[:, :].min(), dtype=np.float32)
                / np.subtract(data[:, :].max(), data[:, :].min(), dtype=np.float32)
            )
            * 255.9
        ).astype(np.uint8)
        return data

    def generate_tfrecord_from_patient(
        self,
        inputFileVolume,
        InputFileLabel,
        is_train_tfrecord,
        ...
        for i in range(self.SLICE_START, self.SLICE_END + 1):
            ...
            ImageEmpty = (
                not self.is_valid_image(frame_0_gray_scale)
                or not self.is_valid_image(frame_1_gray_scale)
                or not self.is_valid_image(frame_2_gray_scale)
                or not self.is_valid_image(frame_3_gray_scale)
            )
            if not ImageEmpty:
                if not is_test_tfrecord:
            ...
            example = tf.train.Example(
                features=tf.train.Features(
                    feature={
                        "image": _bytes_feature(frames.tostring()),
                        "label": _bytes_feature(label.tostring()),
                        "PatientID": _int64_feature(
                            int(self.get_patient_id(inputFileVolume))
                        ),
                        "Slide": _int64_feature(i),
                        "height": _int64_feature(self.HEIGHT),
                        "width": _int64_feature(self.WIDTH),
    ...
    def generate_tfrecords(self):
        ...
        num_of_images_for_train_and_val = (
            self.PATIENT_END_TRAINING - self.PATIENT_START_TRAINING + 1
        )
        images_for_validation = int(
            utils.percentage(
                self.percent_for_validation, num_of_images_for_train_and_val
            )
        )
        # Get random list of files for validation.
        validation_list = random.sample(
            range(self.PATIENT_START_TRAINING, self.PATIENT_END_TRAINING + 1),
            images_for_validation,
        )

        # train and validation
        for i, j in enumerate(
            list(range(self.PATIENT_START_TRAINING, self.PATIENT_END_TRAINING + 1))
        ):
            ...
            if os.path.isfile(InputFileVolume) and os.path.isfile(InputFileLabel):
                if j in validation_list:
                    self.generate_tfrecord_from_patient(
                        InputFileVolume, InputFileLabel, False, True, False
                    )
                else:
                    self.generate_tfrecord_from_patient(
                        InputFileVolume, InputFileLabel, True, False, False
                    )

        # test
        for i, j in enumerate(
            list(range(self.PATIENT_START_TEST, self.PATIENT_END_TEST + 1))
        ):
            InputFileVolume = os.path.join(self.imagePathTS, "BRATS_%03d.nii.gz" % (j))
            if os.path.isfile(InputFileVolume) and os.path.isfile(InputFileLabel):
                self.generate_tfrecord_from_patient(
                    InputFileVolume, "", False, False, True
                )
...
if __name__ == "__main__":
    ...
    fifty_percent_of_labeled = True
    generator = GenerateTFRedord(
        imagePathTR,
        labelPathTR,
        imagePathTS,
        outputDir,
        number_of_volumes_to_process,
        percent_for_validation,
        fifty_percent_of_labeled,
    )
