...
class TFRecordExtractor:
    ...
    def export_without_session(self):
        ...
        for string_record in record_iterator:
            ...
            frames = example.features.feature["image"].bytes_list.value[0]
            label = example.features.feature["label"].bytes_list.value[0]
            PatientID = example.features.feature["PatientID"].int64_list.value[0]
            Slide = example.features.feature["Slide"].int64_list.value[0]
            ...
            outputFile = self.get_output_file_name(
                os.path.basename(self.tfrecord_file),
                Slide,
                0,
                "train" if self.isTraining else "test",
            )
            self.save_image(
                frames[:, :, 0], os.path.join(self.outputFolder, outputFile)
            )
            ...
            self.save_image(
                frames[:, :, 1], os.path.join(self.outputFolder, outputFile)
            )
            print("Generated file: %s" % outputFile)
            ...
            self.save_image(
                frames[:, :, 2], os.path.join(self.outputFolder, outputFile)
            )
            print("Generated file: %s" % outputFile)
            ...
            self.save_image(
                frames[:, :, 3], os.path.join(self.outputFolder, outputFile)
            )
            print("Generated file: %s" % outputFile)

            if self.isTraining:
                outputFile = self.get_output_file_name(
                    os.path.basename(self.tfrecord_file), Slide, 0, "label"
                )
            ...
            while True:
                try:
                    ...
                    if self.isTraining:
                        ...
                        self.save_image(
                            label, os.path.join(self.outputFolder, outputFile)
                        )
                        print("Generated file: %s" % outputFile)
