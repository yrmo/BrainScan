...
def unet(data, training=False, norm_option=False, drop_val=0.5, label_output_size=1):
    ...
    conv2 = tf.layers.conv2d(
        ...
        strides=[1, 1],
        padding="SAME",
        kernel_initializer=tf.initializers.random_normal(stddev=sqrt(2 / (3 * 3 * 64))),
    )
    if norm_option == True:
    ...
    # Bottleneck
    conv_bottle_1 = tf.layers.conv2d(
        pool4,
        1024,
        [3, 3],
        strides=[1, 1],
    ...
    conv_bottle_2 = tf.layers.conv2d(
        conv_bottle_1,
        1024,
        [3, 3],
        strides=[1, 1],
    ...
    drop = tf.cond(
        training,
        lambda: tf.layers.dropout(conv_bottle_2, rate=drop_val),
        lambda: conv_bottle_2,
    )
